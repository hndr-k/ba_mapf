...
std::vector<rosPath> CBS::savePaths() const
{
	std::vector<rosPath> solution_;
	for(int i = 0; i < num_of_agents; i++)
	{
		rosPath current_;
		current_.robot_id = i;
		for (const auto & t : *paths[i])
		{
			current_.x_poses.push_back(search_engines[0]->instance.getColCoordinate(t.location));
			current_.y_poses.push_back(search_engines[0]->instance.getRowCoordinate(t.location));
		}
		solution_.push_back(current_);
	}
