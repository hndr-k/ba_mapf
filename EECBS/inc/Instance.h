...
public:
	...
	Instance(const nav2_costmap_2d::Costmap2D* costmap_, std::vector<rosAgent> agents_ ,
		int num_of_agents = 0, int num_of_rows = 0, int num_of_cols = 0, int num_of_obstacles = 0, int warehouse_width = 0);
	...
	int getDefaultNumberOfAgents() const { return num_of_agents; }
	  ...
	  void printMap() const;
	  void saveMap() const;
	  bool loadMap(const nav2_costmap_2d::Costmap2D* costmap_);	
	  bool loadAgents();
	  bool loadAgents(const nav2_costmap_2d::Costmap2D* costmap_, std::vector<rosAgent> agents_);
